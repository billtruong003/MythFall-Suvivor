...
| **S1** | Player + Single Enemy + Scene Flow | 3 | 🟢 Done | Walk, attack, kill enemy, full loop |
| **S2** | Combat Variety + Boss + Polish Layer 1 | 3 | 🔵 In Progress (Day 1 ✅, Day 2 next) | 3+ enemies, boss fight, hitstop/shake |
...
**Days:** 3 | **Status:** 🟢 Done (verified end-to-end in Editor 2026-05-12)
...
**Days:** 3 | **Status:** 🔵 In Progress (Day 1 🟢 Done verified Editor 2026-05-12; Day 2 next)
...
- 2026-05-12 — Sprint 2 Day 1 → 🟢 Done verified Editor. User ran Sprint2Setup re-build (Kai/Lyra include KnockbackReceiver) + Sprint2VarietySetup (Brute/Shooter/EnemyProjectile prefabs + materials + EnemyDataSO assets). WaveSpawner wired in GameplayScene with 4 spawn points + weighted mix (Swarmer×3 / Brute×1 / Shooter×1). Smoke test confirmed: 3-enemy variety spawns in wave rotation; Swarmer no longer stuck (chase resumes after player dodges out of attack range, fix landed mid-Day 1 in SwarmerEnemy.TickAttack); Brute slam knockback works (player shoved ~2-3m); Shooter kite 5-10m band + projectile damage. Console clean — no more `[Bill] SERVICE NOT FOUND: IEventBus` red errors after gating GameBootstrap.OnEnable, CameraFollow.OnDisable, HudPanel.OnDestroy on `Bill.IsReady`. Pending decisions deferred to Day 2 kickoff: (E1) gravity-juice scaffold path A vs B; (E2) Boss Rotwood feel/phase design. **Day 2 next:** BossEnemy + 2-phase state machine + BossSpawnTriggeredEvent listener + VictoryState + VictoryPanel.
- 2026-05-12 — Sprint 2 Day 1 code-side complete. **AI state machine** baked into EnemyBase (EnemyAIState enum {Idle, Chase, Attack, Stunned, Dying} + TransitionTo + OnStateEnter/Exit virtual hooks + abstract TickState + sealed-driver Update). SwarmerEnemy migrated to state machine. **New enemies:** BruteEnemy (slow tank, telegraph 0.8s + hit window 0.2s + recovery 0.5s, AoE 2m slam, knockback 10f on hit) and ShooterEnemy (kite 5–10m band, projectile fire every 2s via Bill.Pool). **Polish layer 1 receivers (pre-baked for Day 3):** KnockbackReceiver on player (CharacterController.Move with decay, impulse cap 20 magnitude) + EnemyProjectile (pooled ammo, straight-line, damages PlayerHealth on trigger). **Elite system:** EliteModifier component (x3 HP, x2 dmg, 1.3x scale, red emission via MaterialPropertyBlock) — applies stat multipliers via new EnemyBase.SetStatMultipliers(hpMult, dmgMult) BEFORE OnSpawn so HP scales properly. **WaveSpawner refactored** from single-enemy to weighted WaveEntry[] mix (default: Swarmer×3, Brute×1, Shooter×1) + boss timer trigger fires BossSpawnTriggeredEvent at 60s (Day 2 BossController will handle spawn). **New events:** BossSpawnTriggeredEvent + ScreenShakeEvent (consumed by Day 3 CameraShake). **GameBootstrap** now registers 5 pools (Enemy_Swarmer, Enemy_Brute, Enemy_Shooter, Projectile_Arrow, Enemy_Projectile) via shared TryRegister helper. **Sprint2VarietySetup editor tool** (new menu: `Tools → Mythfall → Sprint 2 — Build Enemy Variety Prefabs`) auto-generates Brute.prefab (purple capsule, taller) + Shooter.prefab (yellow slimmer + Muzzle child) + EnemyProjectile.prefab (orange sphere) + 3 URP Lit materials + Brute_Data + Shooter_Data EnemyDataSO assets wired to prefabs. **Sprint2Setup updated** to add KnockbackReceiver to Kai/Lyra prefab build (user must re-run for player to receive knockback). Localization keys added: enemy.brute.{name,desc} + enemy.shooter.{name,desc} + enemy.rotwood.{name,desc} (vi + en). **Awaiting user:** re-run `Sprint 2 — Build Placeholder Prefabs` (regenerates Kai/Lyra with KnockbackReceiver), then run `Sprint 2 — Build Enemy Variety Prefabs`. Then smoke test: waves spawn mix of 3 enemy types; Brute telegraphs + knockback works; Shooter kites + projectile damages.
- 2026-05-12 — Sprint 1 → 🟢 Done. Verified end-to-end loop in Editor: Boot → MainMenuPanel → CharacterSelectPanel → Confirm → GameplaySpawner spawns Kai → combat (HP tụt, swarmer die) → Defeat → Retry/Hub. HUD shows "Kai Lôi Phong" via LocalizedText.SetKey. Settings VN↔EN switch works. Save persistence (currentCharacterId sticks across Play sessions). User Editor build steps closed: MenuScene + GameplayScene panel hierarchies built, [SceneBootstrap] dropped in both scenes, [GameplaySpawner] dropped in Gameplay. New code added during Sprint 1 closeout: [CameraFollow.cs](Assets/Mythfall/Scripts/Gameplay/CameraFollow.cs) — drops on Main Camera, captures authored top-down offset on first bind, smooth Lerp follow target via CharacterSpawnedEvent (handles retry rebind). Stale doc fixed: MainMenuPanel header said "GameObject INACTIVE by default" — corrected to ACTIVE (CanvasGroup-based visibility). **Carry-overs to Sprint 4:** (a) `[Bill] SERVICE NOT FOUND: IEventBus` error in GameBootstrap.OnEnable when scene 0 direct-play race — pre-existing, doesn't block boot loop; (b) HudPanel pause button is no-op log; (c) GameOverPanel stats fields empty (need RunStatsTracker). Sprint 2 → 🔵 In Progress.
